...
function TemplatePreview({ template, userId }) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto my-8 bg-[#111111]/90 backdrop-blur-xl rounded-2xl border border-[#2a2a2a] shadow-xl p-6">
      <h3 className="text-lg font-semibold text-gray-100 mb-4">
        Template Preview
      </h3>

      <div className="bg-[url('/wa-bg.png')] bg-cover bg-center rounded-2xl p-5">
        <div className="bg-[#0f0f0f] rounded-2xl shadow-md overflow-hidden border border-[#2a2a2a]">
          {/* HEADER */}
          {mediaUrl && (
            <div className="relative bg-[#161616] flex justify-center items-center max-h-72">
              {isMediaLoading && (
                <div className="absolute inset-0 bg-[#1a1a1a] animate-pulse flex items-center justify-center text-sm text-gray-400">
                  Loading media…
                </div>
              )}
          ...
          {/* BODY */}
          <div className="p-4 text-gray-100 leading-relaxed whitespace-pre-wrap">
            {(() => {
              const templateText = body?.text || "";
              const exampleValues = body?.example?.body_text?.[0] || [];
          ...
          {buttons?.buttons && (
            <div className="border-t border-[#2a2a2a]">
              {buttons.buttons.map((btn, i) => (
                <button
                  key={i}
                  className="w-full py-3 text-indigo-300 font-medium hover:bg-[#1a1a1a] transition"
                >
                  ➜ {btn.text}
                </button>
              ))}
...
export default function TemplateList() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto text-gray-100">
      <h2 className="text-3xl font-semibold tracking-wider uppercase text-center mb-10 text-gray-100">
        WhatsApp Templates
      </h2>

      {/* Loading */}
      {loading && (
        <div className="flex justify-center py-16 text-gray-400 animate-pulse">
          Loading templates…
        </div>
      )}
      ...
      {/* Table */}
      {!loading && templates.length > 0 && (
        <div className="overflow-x-auto rounded-2xl border border-[#2a2a2a] bg-[#111111]/90 backdrop-blur-xl shadow-lg">
          <table className="min-w-[700px] w-full">
            <thead>
              <tr className="bg-[#161616] text-xs uppercase tracking-wide text-gray-300">
            ...
            <tbody>
              {templates.map((tpl) => (
                <React.Fragment key={tpl.id}>
                  <tr className="border-t border-[#2a2a2a] text-center hover:bg-[#1a1a1a] transition">
                    <td className="p-4 font-medium">{tpl.name}</td>
                    <td className="p-4 text-sm text-gray-300">{tpl.category}</td>
                    <td className="p-4 text-sm text-gray-300">{tpl.language}</td>
                    ...
                    <td className="p-4 text-center">
                      ...
                      >
                        ...
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-[#1f1f1f] transition text-gray-300"
                        ...
                        {/* Popover */}
                        {openMenuId === tpl.id && (
                          <div className="absolute right-0 mt-2 w-44 bg-[#111111] rounded-xl shadow-lg border border-[#2a2a2a] z-50 overflow-hidden">
                            {/* Preview */}
                            <button
                              onClick={() => {
                                setExpanded(
                              ...
                              className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-gray-200 hover:bg-[#1a1a1a]"
                            >
                              <Eye size={16} />
                            ...
                            {tpl.status === "APPROVED" && (
                              <button
                                ...
                                className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-emerald-300 hover:bg-[#122018]"
                              >
                                <Send size={16} />
                                Send
                              </button>
                            ...
                            <button
                              ...
                              className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-red-300 hover:bg-[#2a1111]"
                            >
                              <Trash2 size={16} />
                              Delete
                            </button>
                  ...
                  {expanded === tpl.id && (
                    <tr>
                      <td
                        colSpan="5"
                        className="bg-gradient-to-r from-[#121212] to-[#1a1a1a]"
                      >
                        <TemplatePreview template={tpl} userId={userId} />
                      </td>
                    </tr>
